...
bool startup_db(){
    ...
    for (size_t i = 0; i < num_tables_to_read; i++){
        ...
        for (size_t j = 0; j < num_columns_to_read; j++){
            ...
            load_column_data(&current_table->columns[j], current_table->table_alloc_size, current_table->name);
            ...
            current_table->columns[j].update_struct.del_pos = malloc(sizeof(size_t) * UPDATE_BATCH_SIZE);
            current_table->columns[j].update_struct.del_val = malloc(sizeof(int) * UPDATE_BATCH_SIZE);
            current_table->columns[j].update_struct.ins_val = malloc(sizeof(int) * UPDATE_BATCH_SIZE);
...
/*
* This function write the database to disk. This happens in two parts. First we write the db, tb and column structs to catalogue.cat
* This is in the format [db][tb1][tb1.cola][tb1.colb][tb2][tb2.cola], i.e the db struct followed by each table and all of that tables 
* associated columns. This is done because while reading we first find how many tables there are and then for each table we find 
* how many columns follow it before the next table. 
* should I also free memory here? yeah probs 
*/
...
bool write_db(){
    ...
    // For debugging purposes, should remove this at some point as it should be possible to start the program and quit
    // without creating a db and that shouldn't fail
    assert(g_db!=NULL);
    ...
    for (size_t i = 0; i < num_tables_to_write; i++){
        ...
        for (size_t j = 0; j < num_columns_to_write; j++){
            save_column_data(&current_table->columns[j], current_table->table_alloc_size);
 ...
 * don't need to worry about the fd since that is dealt with in column creation or
 * loading.
 */

bool save_column_data(Column *col, size_t tables_size){
    //could do this async but this isn't performance critical and may cause problems 
    msync(col->data, tables_size, MS_SYNC); 
    if(munmap(col->data, sizeof(int) * tables_size) == -1){
        log_err("failed to munmap column %s, errno %d, %s \n", col->name, errno, strerror(errno));
        return false;
    }
...
bool load_column_data(Column *col, size_t tables_size, char* table_name){
    int path_length = LEN_DATA_PATH + strlen(table_name) + strlen(col->name) + 4 + 2;
    ...
    col->data = mmap(NULL, tables_size * sizeof(int), PROT_WRITE|PROT_READ, MAP_SHARED, fd, 0);
...
bool save_index(Column* column, char* table_name){ //void* index, IndexType index_type, char* table_name, char* col_name){
    int path_length = LEN_DATA_PATH + strlen(table_name) + strlen(column->name) + 4 + 2;
    char col_index_path[path_length];
    sprintf(col_index_path, "%s%s/%s%s", DATA_PATH, table_name, column->name, ".index");
    ...
    if (column->index_type == SORTED){
        ...
        fwrite(sorted_index->col_positions, sizeof(size_t), sorted_index->length, index_file);
    }
...
bool load_btree_index(Column* column, FILE* in_file){
    ...
    // Unlike the BFS traversal, we easily know the number of nodes based on the size
    // of the file.
    ...
    while (curr_idx < seen_nodes){
        ...
        //      all_node_ptrs = realloc(all_node_ptrs, sizeof(BNode*) * (num_nodes + 81));
        //      num_nodes += 81;
        // }
        // printf("num_nodes %ld\n", num_nodes);
        ...
        if (curr_node->is_leaf == false){
            ...
            size_t ptrs_in_node = curr_node->num_elements ; // +1 is the rightmost edge pointer 
            ...
            //     log_err("%s:%d Failed to read all children of node  readnodes:%ld  ptrs_in_node: %ld. errno %d strerror %s\n", __FILE__, __LINE__, read_nodes, ptrs_in_node, errno, strerror(errno));
            //     // return false;
            //     break;
            // }
...
bool load_sorted_index(Column* column, FILE* in_file){
    ...
    if (column->clustered == false){
        if (fread(sorted_index->keys, sizeof(int), sorted_index->length, in_file) < sorted_index->length){
    ...
    sorted_index->col_positions = malloc(sizeof(size_t) * sorted_index->allocated_size);
    if (fread(sorted_index->col_positions, sizeof(size_t), sorted_index->length, in_file) < sorted_index->length){

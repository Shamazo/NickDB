 ...
 * This method returns the original string truncated to where its first comma
 *lies. In addition, the original string now points to the first character after
...
DbOperator* parse_create_tbl(char* create_arguments) {
  ...
  // turn the string column count into an integer, and check that the input is
  // valid.
...
/**
 * This method takes in a string representing the arguments to create a
 *database. It parses those arguments, checks that they are valid, and creates a
 *database.
 **/
...
DbOperator* parse_create_col(char* create_arguments, message* send_message) {
  ...
  char* table_full_name =
      next_token(&create_arguments, &(send_message->status));
...
DbOperator* parse_create_index(char* create_arguments) {
  ...
  if (cluster_type == NULL || idx_type == NULL || col_name == NULL) {
    log_err("%s:%d Bad arguments for create index %s %s %s\n", __FILE__,
            __LINE__, cluster_type, idx_type, col_name);
    return NULL;
  }
  ...
  dbo->operator_fields.create_operator.table = lookup_table(table_name);
  dbo->operator_fields.create_operator.column =
      lookup_column(dbo->operator_fields.create_operator.table, col_name);

  if (dbo->operator_fields.create_operator.table == NULL ||
      dbo->operator_fields.create_operator.column == NULL) {
    free(dbo);
...
DbOperator* parse_create(char* create_arguments, message* send_message) {
  ...
  tokenizer_copy = to_free =
      malloc((strlen(create_arguments) + 1) * sizeof(char));
  char* token;
...
DbOperator* parse_select(char* query_command, message* send_message,
                         char* handle, ClientContext* context) {
  ...
  // need to look up source to see if it is in client context, or a column in a
  // table. need to look up indices in client context only
...
DbOperator* parse_fetch(char* query_command, message* send_message,
  ...
  dbo->operator_fields.fetch_operator.indices =
      index_handle->generalized_column.column_pointer
          .result;  // HOW do I interpret this as a column pointer?
  free(to_free);
  return dbo;
}
...
DbOperator* parse_print(char* query_command, message* send_message,
  ...
  dbo->operator_fields.print_operator = (PrintOperator){
      .columns = col_array, .col_count = num_args, .col_length = col_length};
  free(copy);
  free(to_free);
  return dbo;
}
 ...
 *distribution? How would you add a new command type to parse? What if such
 *command requires multiple arguments?
 **/

DbOperator* parse_avg(char* query_command, ClientContext* context,
                      char* handle) {
  ...
  if (src == NULL) {
    ...
    if (table == NULL) {
      log_err(
          "%s:%d No associated handle in parse_avg and could not find table\n",
          __FILE__, __LINE__);
      free(dbo);
      return NULL;
    }
    ...
    if (src == NULL) {
      log_err(
          "%s:%d No associated handle in parse_avg and could not find column\n",
          __FILE__, __LINE__);
      free(dbo);
      return NULL;
    }
...
DbOperator* parse_min(char* query_command, ClientContext* context,
                      char* handle) {
  char* column_handle = trim_parenthesis(query_command);
  DbOperator* dbo = malloc(sizeof(DbOperator));
  dbo->type = MIN;
  ...
  if (src == NULL) {
    ...
    if (table == NULL) {
      log_err(
          "%s:%d No associated handle in parse_min and could not find table\n",
          __FILE__, __LINE__);
      free(dbo);
      return NULL;
    }
    ...
    if (src == NULL) {
      log_err(
          "%s:%d No associated handle in parse_min and could not find column\n",
          __FILE__, __LINE__);
      free(dbo);
      return NULL;
    }
...
DbOperator* parse_max(char* query_command, ClientContext* context,
                      char* handle) {
  char* column_handle = trim_parenthesis(query_command);
  DbOperator* dbo = malloc(sizeof(DbOperator));
  dbo->type = MAX;
  ...
  if (src == NULL) {
    ...
    if (table == NULL) {
      log_err(
          "%s:%d No associated handle in parse_max and could not find table\n",
          __FILE__, __LINE__);
      free(dbo);
      return NULL;
    }
    ...
    if (src == NULL) {
      log_err(
          "%s:%d No associated handle in parse_max and could not find column\n",
          __FILE__, __LINE__);
      free(dbo);
      return NULL;
    }
...
DbOperator* parse_sum(char* query_command, ClientContext* context,
                      char* handle) {
  char* column_handle = trim_parenthesis(query_command);
  DbOperator* dbo = malloc(sizeof(DbOperator));
  dbo->type = SUM;
  ...
  if (src == NULL) {
    ...
    if (table == NULL) {
      log_err(
          "%s:%d No associated handle in parse_sum and could not find table\n",
          __FILE__, __LINE__);
      free(dbo);
      return NULL;
    }
    ...
    if (src_column == NULL) {
      log_err(
          "%s:%d No associated handle in parse_sum and could not find column "
          "%s in table %s \n",
...
DbOperator* parse_binary_op(char* query_command, ClientContext* context,
                            char* handle) {
  ...
  if (left_col_han == NULL) {
    ...
    dbo->operator_fields.binary_operator.left_column.column_pointer.column =
        left_column;
  } else {
    dbo->operator_fields.binary_operator.left_column =
        left_col_han->generalized_column;
  }
  ...
  if (right_col_han == NULL) {
    ...
    dbo->operator_fields.binary_operator.right_column.column_pointer.column =
        right_column;
  } else {
    dbo->operator_fields.binary_operator.right_column =
        right_col_han->generalized_column;
  }
  return dbo;
}
...
DbOperator* parse_join(char* query_command, ClientContext* context,
  ...
  printf("%s %s %s %s %s \n", val1_name, pos1_name, val2_name, pos2_name,
         query_command);
  if (val1_name == NULL || pos1_name == NULL || val2_name == NULL ||
      pos2_name == NULL) {
...
DbOperator* parse_command(char* query_command, message* send_message,
  ...
  // commands). Either way, you should track the dbo and free it when the
  ...
  //   Note, some commands might want to relay a different status back to the
  //   client.
  ...
  if (strncmp(query_command, "load", 4) == 0) {
    ...
    if (strncmp(query_command, "create", 6) == 0) {
      ...
      if (dbo != NULL) dbo->type = ADD;
    } else if (strncmp(query_command, "sub", 3) == 0) {
      query_command += 3;
      dbo = parse_binary_op(query_command, context, handle);
      if (dbo != NULL) dbo->type = SUB;

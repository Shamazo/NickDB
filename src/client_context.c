...
Column* lookup_column(Table* table, char* col_name) {
    if (g_db == NULL || table == NULL || col_name == NULL)
        return NULL;
...
* The handle may be a handle in the users context or a fully qualified column name. 
*/
...
GeneralizedColumn* lookup_src(char* handle, ClientContext* context){
    ...
    log_err("src table name src %s  column name %s\n", src_table->name, src_column->name);
    if(src_column != NULL){
        // yes this is a bandaid. Somewhere, somehow the src_column->num_rows pointer gets redirected to the wrong table 
        // when I have time I will find how. 
        ...
        GeneralizedColumnHandle* generalized_src_handle = lookup_context(handle_if_not_column, context);
...
bool insert_result_context(Result* result, char* handle_name, ClientContext* context){
    ...
    if (handle_ptr != NULL){
        ...
        if (context->chandles_in_use == context->chandle_slots){
            context->chandle_table = realloc(context->chandle_table, 2 * context->chandles_in_use * sizeof(GeneralizedColumnHandle));
            context->chandle_slots = 2 * context->chandles_in_use;
        }
        context->chandle_table[context->chandles_in_use].generalized_column.column_pointer.result = result; // = *handle_ptr;
        context->chandle_table[context->chandles_in_use].generalized_column.column_type = RESULT;
